...
import { Ship as ShipIcon, Compass, MapPin, RefreshCw, Trash2, Edit3, Check, Radio, Calendar, Scale, Anchor } from 'lucide-react';
...
export const ShipList: React.FC<ShipListProps> = ({
  ...
  const startEditCoords = (ship: Ship) => {
    setEditingShipId(ship.id);
    setEditLat(ship.location.latitude.toString());
    setEditLng(ship.location.longitude.toString());
  };

  const saveEditCoords = (ship: Ship) => {
    const latNum = parseFloat(editLat);
    const lngNum = parseFloat(editLng);

    if (!isNaN(latNum) && !isNaN(lngNum)) {
      onUpdateShipLocation(ship.id, {
        ...ship.location,
        latitude: latNum,
        longitude: lngNum,
    ...
    return (
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center text-slate-400 my-6">
        ...
        <p className="text-xs max-w-md mx-auto">
          Click the <strong className="text-cyan-400">"+ Add Ship"</strong> button above to enter your vessel's Name, IMO, and exact MarineTraffic Latitude & Longitude coordinates.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4 my-6">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-white text-lg flex items-center gap-2">
          Registered Fleet & MarineTraffic Coordinates
        ...
        <span className="text-xs text-slate-400 flex items-center gap-1.5">
          <Radio className="w-3.5 h-3.5 text-emerald-400 animate-pulse" /> Coordinates Active
        </span>
      </div>
              ...
              <div className="bg-slate-950/70 border border-slate-800/80 rounded-xl p-3.5 my-3 text-xs space-y-2">
                <div className="flex items-center justify-between text-slate-400 border-b border-slate-800/80 pb-2">
                  <span className="flex items-center gap-1.5 text-cyan-400 font-bold">
                    <MapPin className="w-3.5 h-3.5" /> MarineTraffic Position Coordinates
                  </span>
                ...
                {editingShipId === ship.id ? (
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[10px] text-slate-400 block mb-0.5">Latitude (° N/S)</label>
                        <input
                          type="text"
                          value={editLat}
                          onChange={(e) => setEditLat(e.target.value)}
                          className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 font-mono text-xs text-white"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] text-slate-400 block mb-0.5">Longitude (° E/W)</label>
                        <input
                          type="text"
                          value={editLng}
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                      ...
                      <button
                        onClick={() => saveEditCoords(ship)}
                        className="px-2.5 py-1 rounded bg-emerald-600 text-white font-bold flex items-center gap-1"
                      >
                        <Check className="w-3.5 h-3.5" /> Save Coords
                      </button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold flex items-center justify-between">
                        ...
                        <button
                          onClick={() => startEditCoords(ship)}
                          className="text-cyan-400 hover:text-cyan-300 font-normal underline flex items-center gap-0.5"
                          title="Plug in exact coordinates manually"
                        >
                          <Edit3 className="w-3 h-3" /> Edit
                        </button>
                      </div>
                      <div className="font-mono text-white font-extrabold text-sm mt-0.5">
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-800/80 text-xs">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  title="Re-pull position from MarineTraffic"
                >
